...
(function ($, window, document) {
        ...
        "two_button": {
            ...
            "fnInit": function (oSettings, nPaging, fnCallbackDraw)
            {
                ...
                if (!oSettings.bJUI)
                {
                    ...
                    nPrevious.innerHTML = "&lArr;";
                    nNext = document.createElement('div');
                    nNext.innerHTML = "&rArr;";
                } else
                {
    ...
    $.fn.dataTable = function (oInit)
    {
        ...
        function _fnDraw(oSettings)
        {
            ...
            if (oSettings.uUniform)
            {
                $.fn.uniform && $("input[type!=hidden], textarea, select, button, .btn, .btn > *", oSettings.oInstance[0]).not('#uniform-undefined').filter(function () {
                    return $(this).parents('[id^="uniform-"]').length == 0 ? true : false;
                }).uniform();
            }
        }
...
if (typeof fervoare.toolbox != 'undefined')
{
    fervoare.toolbox.delRow = function (h)
    {
        var table = $(h).parents('table').dataTable();
        table.fnDeleteRow(table.fnGetPosition($(h).parents('tr')[0]), function () {}, false);
        $(h).parents('tr').children('td').css('borderColor', 'red');
        $(h).parents('tr').fadeOut(500, function () {
            $(h).parents('table').dataTable().fnDraw();
            $(this).remove();
        });
    };
    fervoare.toolbox.updateRow = function (h, c)
    {
        var table = $(h).parents('table').dataTable();
        var k = table.fnGetPosition($(h).parents('tr')[0]);
        if (typeof c != 'undefined')
        {
            table.fnUpdate(c, k, 0);
            fervoare.xform();
        }
    };
    fervoare.toolbox.updateTable = function (h, c)
    {
        var table = $(h).parents('table').dataTable();
        if (typeof c != 'undefined')
        {
...
(function (d, i, g) {
    var e = function (a, c) {
        if (!this instanceof e)
            ...
            if (typeof c == "undefined")
                c = {};
            this.s = d.extend({dt: a, tableTop: 0, tableBottom: 0, redrawTop: 0, redrawBottom: 0, rowHeight: null, viewportHeight: 0, viewportRows: 0, stateTO: null, drawTO: null}, e.oDefaults, c);
            this.dom = {force: g.createElement("div"), scroller: null, table: null};
    ...
    e.prototype = {fnRowToPixels: function (a) {
        ...
        }, fnScrollToRow: function (a, c) {
            a = this.fnRowToPixels(a);
            typeof c == "undefined" || c ? d(this.dom.scroller).animate({scrollTop: a}) : d(this.dom.scroller).scrollTop(a)
            ...
            this.dom.scroller = d("div.dataTables_scrollBody", this.s.dt.nTableWrapper)[0];
            ...
            this.s.rowHeight == "auto" && this._fnCalcRowHeight();
            ...
            this.s.dt.aoDrawCallback.push({fn: function () {
                ...
                sName: "Scroller"});
            this.s.dt.aoStateSave.push({fn: function (c, b) {
                    return b + ',"iScroller":' + a.dom.scroller.scrollTop
                }, sName: "Scroller_State"})
        }, _fnScroll: function () {
            var a = this, c = this.dom.scroller.scrollTop, b;
            this.s.trace && console.log("Scroll: " + c + "px - boundaries: " + this.s.redrawTop + " / " + this.s.redrawBottom + ".  Showing rows " + this.fnPixelsToRow(c) + " to " + this.fnPixelsToRow(c + d(this.dom.scroller).height()) + " in the viewport, with rows " + this.s.dt._iDisplayStart + " to " + this.s.dt._iDisplayEnd + " rendered by the DataTable");
            this._fnInfo();
            ...
            if (c < this.s.redrawTop || c > this.s.redrawBottom) {
                b = parseInt(c / this.s.rowHeight, 10) - this.s.viewportRows;
                if (b < 0)
                    b = 0;
                else if (b + this.s.dt._iDisplayLength > this.s.dt.fnRecordsDisplay())
                    b = this.s.dt.fnRecordsDisplay() - this.s.dt._iDisplayLength;
                else
                    b % 2 !== 0 && b++;
                if (b != this.s.dt._iDisplayStart) {
                    ...
                    if (this.s.dt.oFeatures.bServerSide) {
                        ...
                        this.s.drawTO = setTimeout(function () {
                            a.s.dt._iDisplayStart = b;
                            a.s.dt.oApi._fnCalculateEnd(a.s.dt);
                            a.s.dt.oApi._fnDraw(a.s.dt)
                        }, this.s.serverWait)
                    } else {
                        this.s.dt._iDisplayStart = b;
                        this.s.dt.oApi._fnCalculateEnd(this.s.dt);
                        this.s.dt.oApi._fnDraw(this.s.dt)
                    }
                    this.s.trace && console.log("Scroll forcing redraw - top DT render row: " + b)
                }
            }
        }, _fnDrawCallback: function () {
            var a = this, c = this.dom.scroller.scrollTop;
            this.dom.force.style.height = this.s.rowHeight *
                    this.s.dt.fnRecordsDisplay() + "px";
            var b = this.s.rowHeight * this.s.dt._iDisplayStart;
            if (this.s.dt._iDisplayStart === 0)
                b = 0;
            else if (this.s.dt._iDisplayStart === this.s.dt.fnRecordsDisplay() - this.s.dt._iDisplayLength)
                b = this.s.rowHeight * this.s.dt._iDisplayStart;
            this.dom.table.style.top = b + "px";
            this.s.tableTop = b;
            this.s.tableBottom = d(this.s.dt.nTable).height() + this.s.tableTop;
            this.s.redrawTop = c - this.s.viewportHeight / 2;
            this.s.redrawBottom = this.s.tableBottom - 1.5 * this.s.viewportHeight;
            this.s.trace && console.log("Table redraw. Table top: " +
                    b + "px. Table bottom: " + this.s.tableBottom + ". Scroll boundary top: " + this.s.redrawTop + ". Scroll boundary bottom: " + this.s.redrawBottom);
            setTimeout(function () {
                a._fnInfo.call(a)
            }, 0);
            if (this.s.dt.oFeatures.bStateSave && this.s.dt.oLoadedState !== null && typeof this.s.dt.oLoadedState.iScroller != "undefined")
                if (this.s.dt.sAjaxSource !== null && this.s.dt.iDraw == 2 || this.s.dt.sAjaxSource === null && this.s.dt.iDraw == 1)
                    setTimeout(function () {
                        ...
                        a.s.redrawTop = a.s.dt.oLoadedState.iScroller -
                                a.s.viewportHeight / 2
                    }, 0)
        }, _fnCalcRowHeight: function () {
            var a = this.s.dt.nTable.cloneNode(false), c = g.createElement("tbody"), b = g.createElement("tr"), f = g.createElement("td");
            f.innerHTML = "&nbsp;";
            b.appendChild(f);
            c.appendChild(b);
            a.appendChild(c);
            g.body.appendChild(a);
            this.s.rowHeight = d(b).height();
            g.body.removeChild(a)
        }, _fnInfo: function () {
            var a = this.s.dt, c = this.dom.scroller.scrollTop, b = this.fnPixelsToRow(c) + 1, f = this.fnPixelsToRow(c + d(this.dom.scroller).height()), h = a.fnRecordsTotal();
            c = a.fnRecordsDisplay();
            b = a.fnFormatNumber(b);
            f = a.fnFormatNumber(f);
            h = a.fnFormatNumber(h);
            c = a.fnFormatNumber(c);
            c = a.fnRecordsDisplay() === 0 && a.fnRecordsDisplay() == a.fnRecordsTotal() ? a.oLanguage.sInfoEmpty + a.oLanguage.sInfoPostFix : a.fnRecordsDisplay() === 0 ? a.oLanguage.sInfoEmpty + " " + a.oLanguage.sInfoFiltered.replace("_MAX_", h) + a.oLanguage.sInfoPostFix : a.fnRecordsDisplay() == a.fnRecordsTotal() ? a.oLanguage.sInfo.replace("_START_", b).replace("_END_", f).replace("_TOTAL_", c) + a.oLanguage.sInfoPostFix : a.oLanguage.sInfo.replace("_START_",
                    b).replace("_END_", f).replace("_TOTAL_", c) + " " + a.oLanguage.sInfoFiltered.replace("_MAX_", a.fnFormatNumber(a.fnRecordsTotal())) + a.oLanguage.sInfoPostFix;
            a = a.aanFeatures.i;
            b = 0;
            for (h = a.length; b < h; b++)
                d(a[b]).html(c)
        }};
    e.oDefaults = {trace: false, rowHeight: "auto", serverWait: 200};
    e.prototype.CLASS = "Scroller";
    e.VERSION = "1.0.0";
    e.prototype.CLASS = e.VERSION;
    typeof d.fn.dataTable == "function" && typeof d.fn.dataTableExt.fnVersionCheck == "function" && d.fn.dataTableExt.fnVersionCheck("1.8.0") ? d.fn.dataTableExt.aoFeatures.push({fnInit: function (a) {
            return(new e(a,
                    typeof a.oInit.oScroller == "undefined" ? {} : a.oInit.oScroller)).dom.wrapper
        }, cFeature: "S", sFeature: "Scroller"}) : alert("Warning: Scroller requires DataTables 1.8.0 or greater - www.datatables.net/download")
})(jQuery, window, document);
