...
jQuery.dragandscroll = {
    ...
    update: function (offsetLeft) {
        if (typeof offsetLeft == "undefined") offsetLeft = this.left.outerWidth();
        this.winWidth = $(window).width();
        if (offsetLeft <= this.minWidth) offsetLeft = this.minWidth;
        if (this.winWidth - offsetLeft <= this.minWidth) offsetLeft = this.winWidth - this.minWidth;
...
jQuery.flyingtabs = {
    ...
    movable: function () {
        $.each(
            [this.lastPrepend[0].childNodes[1], this.lastPrepend[1].childNodes[0], this.lastPrepend[1].childNodes[1]],
            function (i, v) {
    ...
    add: function (id, title) {
        if (typeof this.holders[id] == "undefined") return false;
        ...
        this.lastPrepend = $(
            "div:first-child",
            parentObj.prepend(
                '<div class="tab" style="display:none;"><div class="tab-t"><span class="tab-close">&nbsp;</span><span class="tab-refresh">&nbsp;</span>' +
                    title +
                    '</div><div class="tab-cnt"><div class="tab-l"></div><div class="tab-c"><div class="tab-ic"></div></div><div class="tab-r"></div></div><div class="tab-b"></div></div>'
            )
        );
...
jQuery.toolbox = {
    ...
    t2: function (h) {
        ...
        $.each(h.find("a.t2"), function () {
            ...
            if ($.inArray(h.attr("id"), attr) < 0) attr[attr.length] = h.attr("id");
            $(this).click(function (e) {
                e.preventDefault();
                e.stopPropagation();
    ...
    ts: function (h) {
        $.each(h.find("a.ts"), function () {
            ...
            if (!h.find("a.t2")) h.removeAttr("id");
            $(this).click(function (e) {
                e.preventDefault();
                e.stopPropagation();
                parent.removeClass("create");
    ...
    d: function (h) {
        ...
        h.sortable({
            ...
            stop: function (event, ui) {
                ...
                if (arg.start != arg.end) $.dispatch.fetch("task/todo/t_u", arg, div);
                div.remove();
            }
        });
    }
...
jQuery.dispatch = {
    init: function (api) {
        if (typeof api != "undefined") {
            ...
            if (typeof API_URL != "undefined") api = API_URL;
            else {
                $(".noscript p").html(
                    "The HTML template has been broken. This application needs the API_URL javascript variable in order to function.<br/> Fix the template and refresh this page."
                );
                return false;
            }
        }
    ...
    req: function (u, d, w, t, h) {
        ...
        $.ajax({
            ...
            dataType: "json",
            beforeSend: function () {
                this.h = typeof h == "undefined" || h.length == 0 ? $.flyingtabs.add(w, t) : h;
    ...
    fetch: function (a, d, h, s) {
        $.dispatch.req(API_URL + a, { data: typeof s == "undefined" ? JSON.stringify(d) : d }, null, null, h);
    }
};
