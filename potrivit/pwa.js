...
const staticCacheName = "potrivit-0.1.6";

// Installation event
self.addEventListener("install", event => {
    console && console.log("%cpotrivit", "color:purple", "Installing app version 0.1.6...");
});
            ...
            event.respondWith(
                (async () => {
                    try {
                        ...
                        console && console.log("%cpotrivit", "color:purple", "Offline", error);
                    }
                })()
            );
        } else {
                ...
                caches.match(event.request).then(response => {
                    ...
                    var cacheBuster =
                        ...
                        ("markjivko.com" === pathUrl.hostname &&
                            pathUrl.pathname.match(/(?:^\/potrivit\/main\/|\.(?:ico|png|gif|jpe?g|txt|json|woff2)$)/gi));
                    ...
                    return fetch(event.request, { cache: cacheBuster ? "reload" : "no-cache" }).then(response => {
                        ...
                        return caches.open(staticCacheName).then(cache => {
                            console && console.log("%cpotrivit", "color:purple", "DLC " + event.request.url);
...
self.addEventListener("activate", event => {
    console && console.log("%cpotrivit", "color:purple", "Activating app version 0.1.6...");
